...
export function SiteHeader({ current = "home" }: SiteHeaderProps) {
  ...
  const accountMenu = email ? (
    <div className="relative shrink-0">
      <button
        className={`inline-flex min-h-10 items-center gap-2 rounded-full bg-white px-3 py-2 shadow-sm ring-1 ring-[var(--line)] transition hover:bg-[var(--surface-soft)] hover:text-[var(--brand-dark)] ${
          current === "perfil"
    ...
    <header className="sticky top-0 z-20 border-b border-[var(--line)] bg-white/95 backdrop-blur">
      <nav className="mx-auto flex max-w-6xl flex-col gap-3 px-5 py-3 md:px-8 lg:flex-row lg:items-center lg:justify-between lg:px-10">
        <div className="flex items-center justify-between gap-3 lg:contents">
          <Link className="flex min-w-0 flex-1 items-center gap-3 lg:flex-none" href="/">
            <Image
              alt=""
            ...
            <span className="min-w-0 leading-tight">
              <span className="block truncate text-lg font-black text-[var(--brand-dark)]">
                Projeto Brenda
              </span>
              <span className="block truncate text-xs font-bold uppercase tracking-[0.12em] text-[var(--rose)]">
